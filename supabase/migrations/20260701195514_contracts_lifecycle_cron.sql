...
declare
  ...
  v_days_left integer;
begin
  -- countersigned -> active una vez llega start_date (o de inmediato si no tiene)
  update public.contracts
  set status = 'active', updated_at = now()
  where status = 'countersigned'
    and (start_date is null or start_date <= current_date);

  -- active -> expired al pasar end_date
  update public.contracts
  ...
  -- Alertas de renovación 30/15/7 días antes de vencer (idempotente vía contract_alerts_log)
  for v_contract in
    ...
    if v_days_left in (30, 15, 7) then
      if not exists (
        ...
        values (
          ...
          'admin', case when v_days_left <= 7 then 'p0' else 'p1' end,
...
select cron.schedule('process-contract-lifecycle-daily', '30 8 * * *', $cron$select public.process_contract_lifecycle();$cron$);
